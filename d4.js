...
const longestLifeToShortest = inventors.sort(function(a,b) {
...
console.log(longestLifeToShortest);

// 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
// https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
//IN CONSOLE OF THAT PAGE: 
//    const category = document.querySelector('.mw-category')
//    const links = [...category.querySelectorAll('a')]; 
//    const blvds = links.map(link => link.textContent);
//    const de = blvds.filter(name => name.includes('de'));
...
const vehicles = data.reduce((object, vehicle) => {
  if (object[vehicle]) {
    object[vehicle]++
  } else {
    object[vehicle] = 1
